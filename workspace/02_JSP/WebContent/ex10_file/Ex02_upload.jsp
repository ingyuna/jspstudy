...
<body>
	...
	<%
		...
		MultipartRequest multipart = new MultipartRequest(
				...
				"utf-8", 	// 인코딩
				new DefaultFileRenamePolicy()	// 기본파일명변경정책 (동일한 파일을 업로드하면 원래 파일명에 숫자 추가) -> Ex: file1, file2... 
				);	
	%>
	...
	<ul>
		...
		<li>저장된 파일명: <%=multipart.getFilesystemName("filename")%></li>  	<%-- filesystem: 하드디스크를 가르킴 --%>
		<%
			...
			long size = file.length() / 1024;	// length() 메소드는 바이트를 반환하므로 1024로 나누어 KB로 저장
				// String date = file.lastModified()	// long타입의 timestamp값으로 나오니까 -> SimpleDateFormat로 바꿔준다.
