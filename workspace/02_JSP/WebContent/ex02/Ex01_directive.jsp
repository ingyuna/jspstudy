...
<%-- 
	...
	1. page 지시어
		...
		1) <%@ page %>
		2) 페이지의 속성을 저장한다.
		3) WAS(톰캣)가 page 지시어를 해석하고 처리한다. (: Wep Application Server / 더 동적이다) 
		4) 속성 (다 외울 필요는 없다)
			(1) language 	  사용언어
				<%@ page language="java" %>
			(2) contentType   문서타입
	...
	3. taglib 지시어
		1) JSP는 HTML태그와 추가로 확장 태그를 사용한다.
		2) 확장태그를 태그라이브러리라고 한다.
		3) 확장태그로 변수처리, 제어문(분기문, 반복문) 등을 처리한다.
		4) <%@ taglib uri="http://java.sun.com/jsp/jst1/core" prefix="c" %>
