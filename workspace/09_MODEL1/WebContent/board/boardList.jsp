...
<body>
	...
	<div class="container">
		...
		<%
			...
			// 5. 현재 페이지 번호를 이용해 시작 게시글 번호와 종료 게시글 번호 
			/***************** 예시 *******************
			1. 전체 11개 게시글이 있다.
			2. 한 페이지에 3개의 게시글을 표시한다.
			page = 1, beginRecord = 1, endRecord = 3
			page = 2, beginRecord = 4, endRecord = 6
			page = 3, beginRecord = 7, endReocrd = 9
			page = 4, beginRecord = 10, endRecord = 11   --> 마지막 4페이지에는 게시글 2개가 올라온다. 전체가 11개니까.
			******************************************/
			// 시작 게시글번호 = (현재페이지번호 -1) * 페이지당게시글수 + 1
			int beginRecord = (pageVO.getPage() -1) * pageVO.getRecordPerPage() + 1;  // (현재페이지번호 -1) * 페이지당레코드수 + 1
			pageVO.setBeginRecord(beginRecord);
			// 종료레코드번호 = 시작게시글번호 + 페이지당레코드수 -1
			// 단, 종료레코드번호와 전체레코드수 중 작은 값을 종료레코드번호로 사용한다.
			int endRecord = beginRecord + pageVO.getRecordPerPage() -1;
			endRecord = (endRecord < totalRecord) ? endRecord : totalRecord;
			pageVO.setEndRecord(endRecord);		// 이제 저장하기.
			...
			page = 6~10, beginRecord = 6, endPage = 10
			...
			******************************************/
			...
			pageContext.setAttribute("list", list);		// pageContext에 담는 이유 : EL로 사용하기 위해 
		%>
		<table>
			<thead>
				<tr>
