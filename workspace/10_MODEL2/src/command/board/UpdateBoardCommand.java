...
public class UpdateBoardCommand implements BoardCommand {
	...
	public ModelAndView execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// 첨부가 있으면 업로드 처리
			multipartRequest = new MultipartRequest(request, 		// request가 multipart로 바뀌어서 나온다.
													realPath,
													1024 * 1024 * 10,
													"utf-8",
													new DefaultFileRenamePolicy());	  // 동일한 이름이 나왔을때.
			// 새로 첨부하려는 파일
			...
			dto.setIdx(Long.parseLong(multipartRequest.getParameter("idx")));	// multipart에서 꺼내야지 request에서 꺼내면 안된다 -> 그럼 null값 떨어짐.
