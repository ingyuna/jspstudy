	...
	<title>Insert title here</title>
		...
		<script>
			...
			$(document).ready(function(){
				...
				// 이벤트
				$('#update_pw_btn').click(function(){		// '업데이트 패스워드 버튼'을 클릭했을 때,
					updatePw();
				})
				...
				// 함수
				function updatePw() {
					if ('${loginUser.pw}' != $('#pw0').val()) {		// 현재 비밀번호가 다를때,
						alert('현재 비밀번호를 확인하세요.');
						return;
					} else if ('${loginUser.pw}' == $('#pw').val()) {		// 새 비밀번호가 현재 비밀번호와 같을 때,
						alert('현재 비밀번호와 동일한 비밀번호입니다.');
						return;
					} else if ($('#pw').val() == '') {	// 비밀번호 정규식으로 변경
					 	alert('새 비밀번호를 확인하세요.');
						return;
					} else if ($('#pw').val() != $('#pw1').val()) {		// 새 비밀번호와 새 비밀번호 확인이 다를 때,
						alert('비밀번호를 확인하세요.');
						return;
					}
					$.ajax({
				...
				function deleteMember() {
					...
					$.ajax({	
						...
						type: 'post',
						data: 'no=${loginUser.no}',			// 삭제할 사람이 누군지 알아야하니까 
						dataType: 'json',
						success: function(obj) {
							if(obj.result == 1) {
